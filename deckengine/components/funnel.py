...
from ..core.fit_text import WRAP_SAFETY, Span, fit_text
...
_END_FRAC_CAP = 0.55  # auto-raise ceiling when labels need wider low bands
...
class Funnel(Component):
    ...
    def _fracs(self, data: FunnelSpec, width: int,
        ...
        # default taper auto-raises the end frac (cap 0.55) when a low
        # stage's label would not fit its band at full size — the label
        # keeps a whole line instead of shrinking/ellipsizing early
        end = _END_FRAC
        if n > 1 and width > 0:
            pad = ctx.theme.spacing(_PAD_MULT)
            for i, stage in enumerate(data.stages[1:], start=1):
                lw = ctx.measurer.span_width_emu(
                    Span(stage.label, bold=True), ctx.font("body"),
                    ctx.size("small"))
                req = (lw / WRAP_SAFETY + 2 * pad) / width
                # linear taper: frac_i = 1 - (1-end)*i/(n-1) >= req
                end_i = 1.0 - (1.0 - req) * (n - 1) / i
                end = max(end, min(_END_FRAC_CAP, end_i))
        step = (_FRAC_MAX - end) / max(1, n - 1)
    ...
    def _plan(self, data: FunnelSpec, width: int, ctx: RenderContext) -> _Plan:
        ...
        band_ws = tuple(round(width * f)
                        for f in self._fracs(data, width, ctx))
        n = len(data.stages)
