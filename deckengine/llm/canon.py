...
CANON: dict[str, CanonForm] = {f.id: f for f in [
    ...
    _C("combo_line_column", "trend", "two units on one timeline",
       "gap", None),
    ...
    _C("pareto", "distribution", "the vital few (80/20) drawn",
       "gap", None),
    # -- relationship -------------------------------------------------------
    _C("scatter", "relationship", "does X relate to Y", "gap", None),
    _C("bubble", "relationship",
       "X vs Y with size as the third variable (BCG engine)", "gap", None),
    ...
    _C("onion", "diagram", "concentric layers (core -> periphery)",
       "gap", None),
    _C("venn", "diagram", "overlap between 2-3 sets", "gap", None),
    ...
    _C("staircase", "diagram", "step-wise value build to an outcome",
       "gap", None),
    _C("hub_spoke", "diagram", "center + radiating elements", "gap", None),
